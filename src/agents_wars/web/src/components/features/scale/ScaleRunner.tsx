...
export default function ScaleRunner({ agents }: Props) {
  ...
  const [providerModel, setProviderModel] = useState<{ provider: Provider; model: string }>({ provider: "openai", model: "" });
  ...
  const canSubmit = useMemo(() => {
    ...
    const base = !submitting && !!agentId && providerModel.model.trim().length > 0 && runs >= 1 && runs <= 10;
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const { usdIn, usdOut } = estimateCostUsdFromUsage(providerModel.provider, providerModel.model || "", { inputTokens, outputTokens });
  ...
  async function onStart(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        let parsed: any = null;
        try { parsed = JSON.parse(tx); } catch {}
          ...
          (async () => {
            try {
              ...
              if (rr.ok) {
                ...
                setReport({ summary: j.summary, revisedPrompt: j.revisedPrompt, rationale: (j.stats?.rationale as string) ?? undefined, stats: j.stats });
              }
            } catch {}
          })();
          return;
  ...
  const data = (job?.data ?? null) as null | { runId: string; total: number; succeeded: number; failed: number; conversationIds?: string[] };
  const [convOpen, setConvOpen] = useState<Record<string, boolean>>({});
  const [convMsgs, setConvMsgs] = useState<Record<string, { id: string; role: "system" | "user" | "assistant" | "tool"; content: string; createdAt: string }[]>>({});
  ...
  async function toggleConversation(conversationId: string) {
    ...
    if (nextOpen && !convMsgs[conversationId]) {
      try {
        const res = await fetch(`/api/battles/${conversationId}/messages?page=1&limit=100`, { cache: "no-store" });
        if (!res.ok) return;
        const json = (await res.json()) as { items: { id: string; role: "system" | "user" | "assistant" | "tool"; content: string; createdAt: string }[] };
    ...
    <form onSubmit={onStart} className="space-y-4">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <label className="block text-sm text-slate-200">
          Agent
          <select value={agentId} onChange={(e) => setAgentId(e.target.value)} className="mt-1 w-full rounded border border-slate-600 bg-slate-800 p-2 text-slate-100">
            {agentOptions.map((a) => (
        ...
        <div>
          <ProviderModelSelector provider={providerModel.provider} model={providerModel.model} onChange={setProviderModel} />
      ...
      <label className="block text-sm text-slate-200">
        ...
        <textarea value={systemPrompt} onChange={(e) => setSystemPrompt(e.target.value)} className="mt-1 w-full rounded border border-slate-600 bg-slate-800 p-2 text-slate-100" rows={3} />
      </label>

      <label className="block text-sm text-slate-200">
        User message (optional)
        <textarea value={userMessage} onChange={(e) => setUserMessage(e.target.value)} className="mt-1 w-full rounded border border-slate-600 bg-slate-800 p-2 text-slate-100" rows={2} />
      </label>

      <label className="block text-sm text-slate-200">
        Runs (1–10)
        <input type="number" min={1} max={10} value={runs} onChange={(e) => setRuns(Number(e.target.value))} className="mt-1 w-32 rounded border border-slate-600 bg-slate-800 p-2 text-slate-100" />
      </label>
        ...
        <div className="rounded bg-slate-800 p-3">
          ...
          <div className="mt-1 h-2 w-full overflow-hidden rounded bg-slate-700">
            <div
              ...
              style={{ width: `${Math.min(100, Math.max(0, maxBudgetUsd && parseFloat(maxBudgetUsd) > 0 ? (estimatedUsd / parseFloat(maxBudgetUsd)) * 100 : 0))}%` }}
              aria-valuemin={0}
              aria-valuemax={100}
              aria-valuenow={maxBudgetUsd && parseFloat(maxBudgetUsd) > 0 ? Math.min(100, (estimatedUsd / parseFloat(maxBudgetUsd)) * 100) : 0}
              role="progressbar"
              aria-label="Estimated budget usage"
            />
          </div>
          {maxBudgetUsd && parseFloat(maxBudgetUsd) > 0 && estimatedUsd > parseFloat(maxBudgetUsd) && (
      ...
      <div className="flex items-center gap-3">
        <button type="submit" disabled={!canSubmit} className="rounded bg-blue-600 px-4 py-2 text-white disabled:cursor-not-allowed disabled:opacity-60">
      ...
      {(terminal && job?.status === "succeeded" && data) || (process.env.NEXT_PUBLIC_E2E_MODE === "1" && report) ? (
                    ...
                    {convOpen[cid] && (
                      <div id={`conv-${cid}`} className="mt-2 rounded bg-slate-950 p-2">
                        <pre className="whitespace-pre-wrap text-xs text-slate-200">
                          {(convMsgs[cid] || []).map((m) => `[${m.role}] ${m.content}`).join("\n") || "Loading..."}
                        </pre>
                      </div>
                    )}
                  </li>
            ...
            <div className="rounded bg-slate-800 p-3">
              ...
              {(statSucceeded !== undefined || statFailed !== undefined) && (
                <div className="mb-3 flex flex-wrap items-center gap-2">
                  {statSucceeded !== undefined && (
                    <span className="rounded bg-green-900/40 px-2 py-0.5 text-xs text-green-300">Succeeded: {statSucceeded}</span>
                  )}
                  {statFailed !== undefined && (
                    <span className="rounded bg-red-900/40 px-2 py-0.5 text-xs text-red-300">Failed: {statFailed}</span>
                  )}
                  {(() => {
                    ...
                    if (typeof total === "number" && total > 0 && typeof succ === "number") {
                      ...
                      return <span className="rounded bg-slate-700 px-2 py-0.5 text-xs text-slate-200">Win rate: {winRate}%</span>;
                    }
                    return null;
                  })()}
                </div>
              ...
              <div className="mb-3 grid grid-cols-2 gap-3">
                ...
                <div className="rounded bg-slate-900 p-2">
                  ...
                  <div className="text-sm text-slate-100">${(statActualUsd ?? actualUsd).toFixed(4)}</div>
                </div>
              </div>
              {report.summary && (
              ...
              {report.revisedPrompt && (
                <div className="mt-3">
                  ...
                  <pre className="whitespace-pre-wrap rounded bg-slate-950 p-2 text-xs text-slate-100">{report.revisedPrompt}</pre>
                  <div className="mt-2 flex gap-2">
                    <button
                      type="button"
                      onClick={async () => {
                        try { await navigator.clipboard.writeText(report.revisedPrompt || ""); } catch {}
                      }}
                      className="rounded bg-slate-700 px-2 py-1 text-xs text-slate-200 hover:bg-slate-600"
                    >
                      Copy prompt
                    ...
                    {report.rationale && (
                      <button
                        type="button"
                        onClick={async () => {
                          try { await navigator.clipboard.writeText(report.rationale || ""); } catch {}
                        }}
                        className="rounded bg-slate-700 px-2 py-1 text-xs text-slate-200 hover:bg-slate-600"
                      >
                        Copy rationale
