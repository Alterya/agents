...
describe("scale start/status/report routes", () => {
  it("starts a scale run and streams status to completion", async () => {
    ...
    const resReport = await (report as any)(
      new Request(`http://localhost/api/scale/${id}/report`) as any,
      { params: { id } } as any,
    );
  ...
  it("enforces runs cap and idempotency", async () => {
    const idem = "idem-123";
    const first = await (start as any)(
      new Request("http://localhost/api/scale/start", {
        method: "POST",
    ...
    const second = await (start as any)(
      new Request("http://localhost/api/scale/start", {
        method: "POST",
        headers: { "content-type": "application/json", "x-idempotency-key": idem },
        body: JSON.stringify({
    ...
    // cap validation
    const overCap = await (start as any)(
      new Request("http://localhost/api/scale/start", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
